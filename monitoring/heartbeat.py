...
def healthcheck():
	...
	r = requests.get("http://localhost:8080/?q=*", verify=False)
	...
	if num_sources != num_source_buttons:
		return "Some buttons are missing!  The buttons that should be shown are: {}".format(', '.join(KNOWN_DOC_TYPES))
	...
	if not match:
		return "Failure while parsing page, couldn't find 'Display limited to etc...' in the source"
	...
	if num_hits != num_results:
		return "The number of results on the page is incorrect. Expected {} but had {}".format(num_results, num_hits)
